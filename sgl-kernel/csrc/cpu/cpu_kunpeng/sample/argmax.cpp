...
void argmax_kunpeng(
    const at::Tensor prob_distribution, at::Tensor token_ids, at::Tensor token_probs, int64_t height, int64_t width)
{
    CHECK_LAST_DIM_CONTIGUOUS_INPUT(prob_distribution);
    CHECK_DIM(2, prob_distribution);
    TORCH_CHECK(prob_distribution.scalar_type() == at::kBFloat16,
        "prob_distribution must be bfloat16, got ", prob_distribution.scalar_type());
    TORCH_CHECK(token_ids.scalar_type() == at::kLong, "token_ids must be int64, got ", token_ids.scalar_type());
    TORCH_CHECK(token_probs.scalar_type() == at::kFloat, "token_probs must be float32, got ", token_probs.scalar_type());
    ...
    at::parallel_for(0, height, 1, [&](int64_t start, int64_t end) {
        int64_t vl = svcnth();
        auto pg = svptrue_b8();
        for (int64_t hi = start; hi < end; hi++) {
            ...
            for (int64_t wi = 0; wi < width; wi += vl) {
                svbfloat16_t v = svld1(pg, reinterpret_cast<const bfloat16_t*>(prob_distribution_ptr) + hi * width + wi);

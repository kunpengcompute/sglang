...
Graph::Graph(std::vector<StorageBuf> storages,
                     ...
                     torch::Tensor external_pool)
{
    ...
    for (int i = 0; i < total_ops_; ++i) {
        op_names_.push_back(
            op_records_[i].profile_name.empty()
    ...
    for (const auto& [idx, born, death, size] : intervals) {
        ...
        for (const auto& pe : placed) {
            if (intervals_overlap(born, death, pe.born, pe.death)) {
                size_t aligned_end = (pe.offset + pe.size + MEMORY_ALIGNMENT - 1)
        ...
        for (size_t cand : candidates) {
            ...
            if (valid) { best_offset = cand; break; }
        }
...
void Graph::precompute_replay()
{
    ...
    for (size_t vid = 0; vid < views_.size(); ++vid) {
        ...
        if (static_cast<int>(vid) < num_inputs_)
            continue;
        ...
        if (!storage.data_ptr) {
            ...
            cached_tensors_[vid] = torch::empty(
                view.shape,
                torch::TensorOptions().dtype(dtype).device(torch::kCPU));
            continue;
        }

        void* ptr = static_cast<char*>(storage.data_ptr) +
                    view.storage_offset * view.element_size;
        auto dtype = static_cast<c10::ScalarType>(view.scalar_type);
        cached_tensors_[vid] = torch::from_blob(
            ptr, view.shape, view.strides,
            torch::TensorOptions().dtype(dtype).device(torch::kCPU));
    }
    ...
    for (int op_idx = 0; op_idx < total_ops_; ++op_idx) {
        const auto& op = op_records_[op_idx];
        int nvids = static_cast<int>(op.input_view_ids.size() +
                                     op.output_view_ids.size());
        ...
        if (!info || !info->dispatch_fn)
            TORCH_CHECK(false, "precompute_replay: op '", op.op_name, "' not registered");
    ...
    for (int op_idx = 0; op_idx < total_ops_; ++op_idx) {
        ...
        for (int vid : op.input_view_ids)  flat_vids_[pos++] = vid;
        for (int vid : op.output_view_ids) flat_vids_[pos++] = vid;
    }
...
void Graph::hold_fixed(const std::unordered_map<int, torch::Tensor>& fixed)
{
    ...
    for (const auto& [pos, tensor] : fixed) {
        ...
        TORCH_CHECK(tensor.defined(),
                    "hold_fixed: tensor at position ", pos, " is undefined");
...
std::vector<torch::Tensor> Graph::run(const std::vector<torch::Tensor>& inputs)
{
    ...
    TORCH_CHECK(static_cast<int>(inputs.size()) == n,
                "Graph::run: expected ", n, " inputs, got ", inputs.size());
    ...
    for (int i = 0; i < n; ++i) {
        ...
        TORCH_CHECK(inputs[i].defined(),
                    "Graph::run: input at position ", i, " is undefined");
        cached_tensors_[vid] = inputs[i];

        storages_[vid].data_ptr =
            static_cast<char*>(inputs[i].data_ptr()) -
            inputs[i].storage_offset() * inputs[i].element_size();

        // Update view shape/strides for dynamic batch sizes
        views_[vid].shape.assign(
            inputs[i].sizes().begin(), inputs[i].sizes().end());
        views_[vid].strides.assign(
            inputs[i].strides().begin(), inputs[i].strides().end());

        // Sync alias views to the new data_ptr
        for (int alias_vid : input_alias_vids_[i]) {
            const auto& av = views_[alias_vid];
            void* ptr = static_cast<char*>(storages_[vid].data_ptr) +
                        av.storage_offset * av.element_size;
            cached_tensors_[alias_vid] = torch::from_blob(
                ptr, av.shape, av.strides,
                torch::TensorOptions()
                    .dtype(static_cast<c10::ScalarType>(av.scalar_type))
                    .device(torch::kCPU));
        }
    }

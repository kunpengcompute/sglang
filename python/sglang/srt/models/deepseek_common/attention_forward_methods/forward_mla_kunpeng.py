...
class DeepseekMLAKunpengForwardMixin:
    ...
    def forward_absorb_prepare_kunpeng(
        ...
        if self.q_lora_rank is not None:
            ...
            norm_int8 = norm_int8_and_scale.view(torch.int8).as_strided(int8_shape, int8_strides)

            f32_shape = (batch_size, 1)
            f32_strides = (row_bytes // 4, 1)
            scale_start_offset = dim
            norm_scale = norm_int8_and_scale[scale_start_offset:].view(torch.float32).as_strided(f32_shape, f32_strides)

            torch.ops.sgl_kernel.quant_kunpeng(hidden_states, norm_int8, norm_scale)

            # qkva
            m,n,k = batch_size, self.q_lora_rank + self.kv_lora_rank + self.qk_rope_head_dim, dim
            tile_m, tile_n, tile_k = torch.ops.sgl_kernel.igemm_find_optimal_tiling_plan_decode(m, n, k, 32)

            qkva = torch.empty([m, n],dtype = torch.bfloat16)

            pack_a = torch.empty_like(norm_int8)
            torch.ops.sgl_kernel.s8_gemm_pack_kunpeng(norm_int8.contiguous(), pack_a, tile_m, tile_k)

            pack_w = torch.empty_like(self.fused_qkv_a_proj_with_mqa.weight)
            torch.ops.sgl_kernel.s8_gemm_pack_kunpeng(self.fused_qkv_a_proj_with_mqa.weight.contiguous(), pack_w, tile_n, tile_k)

            workspace_size = batch_size *  (self.q_lora_rank + self.kv_lora_rank + self.qk_rope_head_dim) * 32
            workspace = torch.empty(workspace_size, dtype=torch.bfloat16)


            torch.ops.sgl_kernel.s8_s8_packed_gemm_bf16_dq_decode_kunpeng(pack_a, pack_w, self.fused_qkv_a_proj_with_mqa.weight_scale.view(-1), norm_scale.contiguous().view(-1), qkva, workspace, 32)
            ...
            torch.ops.sgl_kernel.rmsnorm_quant_kunpeng(q, self.q_a_layernorm.weight,self.q_a_layernorm.variance_epsilon, qa_norm, qa_norm_scale.view(-1))
            ...
            torch.ops.sgl_kernel.s8_gemm_pack_kunpeng(qa_norm.contiguous(), pack_a, tile_m, tile_k)

            pack_w = torch.empty_like(self.q_b_proj.weight)
            torch.ops.sgl_kernel.s8_gemm_pack_kunpeng(self.q_b_proj.weight.contiguous(), pack_w, tile_n, tile_k)

            workspace_size = batch_size *  (self.q_lora_rank + self.kv_lora_rank + self.qk_rope_head_dim) * 32
            workspace = torch.empty(workspace_size, dtype=torch.bfloat16)


            torch.ops.sgl_kernel.s8_s8_packed_gemm_bf16_dq_decode_kunpeng(pack_a, pack_w, self.q_b_proj.weight_scale.view(-1), norm_scale.contiguous().view(-1), out, workspace, 32)
            q = out.view(-1, self.num_local_heads, self.qk_head_dim)

            # kv_norm
            k_nope = latent_cache[..., : self.kv_lora_rank]
            k_nope_out = torch.empty_like(k_nope)
            torch.ops.sgl_kernel.rmsnorm_kunpeng(k_nope, self.kv_a_layernorm.weight, self.kv_a_layernorm.variance_epsilon, k_nope_out)
            ...
            if self.rotary_emb is not None:
                ...
                torch.ops.sgl_kernel.rope_kunpeng(positions, q_pe, k_pe, q_out_kp, k_out_kp, self.rotary_emb.cos_sin_cache)
                q_pe = q_out_kp
                k_pe = k_out_kp
